...
// Structure(구조체)는 관계가 있는 것들을 묶어서 표현 가능
// Class : Reference Types(이미 생성된 인스턴스를 새로운 변수에 할당할 때 하나의 인스턴스를 바라보고 있음)
// Structure : Value Types(새로운 변수에 할당될 때 기존에 있던 것을 복사해서 새로운 인스턴스를 생성함)
